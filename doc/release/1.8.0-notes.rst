...
Partial sorting by index ``k`` sorts the ``k`` smallest elements into the front of
an array.  All elements before ``k`` are then smaller or equal than the value in
position ``k`` and all elements following ``k`` are then greater or equal than the
value in position ``k`` but the ordering of these values is undefined.

The value of the position ``k`` is then `kth order statistic` of the array. A
sequence of ``k`` indices can be provided to obtain several order statistics at
once via iterative partitioning.

Partition has a linear time complexity of ``O(n)`` compared to ``O(n log(n))`` of a
full sort.
